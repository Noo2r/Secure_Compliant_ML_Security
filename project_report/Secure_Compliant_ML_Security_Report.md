...
```mermaid
flowchart TD
    A[Raw IEEE-CIS Data\ntrain_transaction.csv + train_identity.csv] --> B[Milestone 1\nData Classification and Security\nFernet field-level encryption]
    B --> C[Encrypted, Classified Dataset\ndata/encrypted/encrypted_dataset.csv]
    C --> D[Module 1\nValidation and Data Contract Checks\n9 checks, Ch. 7]
    D --> E[Module 2\nFeature Engineering\n6 leakage-safe transformers, Ch. 8]
    E --> F[Module 2\nFeature Selection\n466 to 150 features, Ch. 8]
    F --> G[Module 3\nModel Training and Optuna Tuning\n5 model families, Ch. 9]
    G --> H[Module 3\nModel Evaluation and Selection\nLightGBM selected, Ch. 9]
    H --> I[Module 4\nDifferential Privacy Evaluation\nDP-SGD + RDP accounting, Ch. 10]
    H --> J[Module 5\nFairness Evaluation\nSPD / DIR / EOD / AOD, Ch. 11]
    H --> K[Model Artifact\nInferenceBundle joblib, Ch. 12]
    K --> L[Milestone 3\nSecure FastAPI Inference Service\nJWT auth + rate limiting, Ch. 13]
    L --> M[Milestone 3\nDocker and Kubernetes Deployment Design\nCh. 14]
    M --> N[Milestone 3\nAzure Key Vault / AKS / WAF\nTarget architecture, not deployed, Ch. 15]
    K --> O[Milestone 5\nDrift Monitoring\nEvidently, Ch. 19]
    O --> P[Milestone 5\nWORM Storage and CI/CD\nDry-run verified, Ch. 20-21]
    B -.audited by.-> Q[Milestone 4\nCompliance QA\nGDPR / HIPAA / ISO 27001, Ch. 18]
    N -.audited by.-> Q
```
...
flowchart LR
    subgraph Untrusted
        U1[Raw dataset files]
        U2[External API client]
    end
    subgraph "Trusted: Data Pipeline"
        T1[Milestone 1 encryption]
        T2[Module 1 validation]
        T3[Modules 2-5 feature/model pipeline]
    end
    subgraph "Trusted: Serving"
        T4[FastAPI app]
        T5[LightGBM InferenceBundle]
    end
    subgraph "Target: Azure (not provisioned)"
        AZ1[Key Vault]
        AZ2[AKS private cluster]
        AZ3[Application Gateway WAF]
    end
    U1 -->|trust boundary: unverified input| T1
    T1 -->|trust boundary: independently re-verified, Ch.7| T2
    T2 --> T3
    T3 --> T5
    U2 -->|trust boundary: authenticated + rate-limited, Ch.13| T4
    T4 --> T5
    T4 -.target.-> AZ1
    T4 -.target.-> AZ2
    AZ3 -.target.-> AZ2
...
```mermaid
flowchart TD
    A[Secured dataset] --> B[Validation\nMod. 1]
    B --> C[EDA Profiling]
    ...
    I --> J{Best model\nby holdout PR-AUC}
    J --> K[Selected: LightGBM]
    K --> L[Differential Privacy Variant]
    K --> M[Fairness Audit]
    K --> N[InferenceBundle Persistence]
```
...
```mermaid
flowchart TD
    subgraph "Implemented and locally run"
        D1[Docker multi-stage build\nnon-root user]
        D2[uvicorn ASGI server\nFastAPI app]
    end
    subgraph "Designed target: Kubernetes manifests"
        K1[Namespace: fraud-inference]
        K2[Deployment: 3 replicas\nreadOnlyRootFilesystem]
        K3[Service: ClusterIP]
        K4[NetworkPolicy: default-deny + explicit allow]
    end
    subgraph "Designed target: Azure -- NOT deployed"
        A1[Application Gateway\nWAF_v2, Prevention mode]
        A2[AKS private cluster]
    ...
    K4 --> K2
    A1 --> A2
    K2 -.would run inside.-> A2
    A2 -.workload identity.-> A3
    A2 --> A4
```
...
```mermaid
flowchart TD
    A[Secured, validated dataset] --> B[Chronological 80/20 split\nby TransactionDT]
    B --> C[Fit engineering transformers\non TRAIN fold only]
    C --> D[Transform train and test]
    D --> E[Extract metadata columns\nTransactionID, card6, card4, DeviceType]
    E --> F[4-method feature selection\non TRAIN fold only]
    F --> G[SelectedColumnsTransformer\npersisted]
    G --> H[Preprocessing pipeline\nimpute / scale / one-hot encode]
    H --> I[Sanity check:\ncomposed vs. manually-chained output]
    I --> J[Persist 5 joblib artifacts\n+ processed CSVs]
```
...
sequenceDiagram
    ...
    participant Pred as /api/v1/inference/predict
    participant RL as Rate Limiter
    participant V as Pydantic Schema
    participant M as InferenceBundle
    participant L as Audit Log

    C->>Auth: POST username + password
    Auth->>Auth: verify bcrypt hash
    Auth-->>C: JWT (15 min expiry)
    C->>Pred: POST + Bearer JWT + TransactionFeatures
    Pred->>RL: check 30 req/min
    RL-->>Pred: allow / 429
    Pred->>Pred: decode + validate JWT, check role
    Pred->>V: validate schema (extra=forbid)
    V-->>Pred: validated TransactionFeatures
    Pred->>Pred: assemble ML-ready DataFrame
    Pred->>M: predict_proba(frame)
    M-->>Pred: fraud probability
    Pred->>Pred: apply bundle's own threshold
    Pred->>L: log request_id, subject, role (never raw features)
    Pred-->>C: PredictionResponse
```
...
```mermaid
flowchart TD
    Internet((Internet)) -->|TLS 1.2+| AppGW[Application Gateway WAF_v2\nPrevention mode, OWASP CRS 3.2]
    AppGW -->|HTTPS, NSG-restricted| AKS[AKS private cluster\nRBAC + Workload Identity]
    AKS -->|3 replicas, NetworkPolicy default-deny| Pods[fraud-inference-api pods]
    Pods -->|private endpoint only, OIDC identity| KV[Key Vault\npublic access disabled]
    Pods --> LA[Log Analytics /\nDefender for Cloud]
    AdminIPs[Allow-listed admin IPs] -.RBAC.-> AKS
```
...
```mermaid
flowchart TD
    A[Reference dataset:\nreal train.csv or synthetic fallback] --> C[Feature manifest\nreports_m2/feature_manifest.json]
    B[Current dataset:\nreal test.csv or synthetic fallback] --> C
    C --> D[Resolve real column names\nnumeric__ / categorical__ prefixes]
    D --> E[Evidently DataDriftPreset]
    E --> F[Per-feature drift results]
    F --> G[Dataset-level drift decision\nok / warning / critical]
    G --> H[drift_report.html\ndrift_summary.csv\ndrift_status.json]
    H --> I[CI/CD decision\nCh. 21]
```
...
```mermaid
flowchart TD
    A[Push / PR / daily schedule / manual dispatch] --> B[drift-monitor job]
    B --> C{Real processed data\npresent in checkout?}
    C -->|No, gitignored| D[Synthetic-data fallback]
    C -->|Yes| E[Real data]
    D --> F[Run drift_monitor.py]
    E --> F
    F --> G[Upload drift-report artifact]
    G --> H{On main AND\ndrift-monitor succeeded?}
    H -->|Yes| I[worm-upload job]
    H -->|No| J[Skip worm-upload]
    I --> K{Azure secrets\nconfigured?}
    K -->|No| L[--dry-run fallback]
    K -->|Yes| M[Real upload]
    G --> N[alert-on-drift job]
    N --> O{Critical AND real data?}
    O -->|Yes| P[Fail pipeline]
    O -->|No| Q[Warning annotation only]
```
